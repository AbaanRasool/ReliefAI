...
import { ChevronDown, Circle, Menu, X } from "lucide-react";
...
function isActiveRoute(pathname: string, href: (typeof NAV_LINKS)[number]["href"]) {
...
export function Navbar() {
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50">
      <div className="bg-gray-950 border-l-4 border-red-600">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="h-16 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 min-w-0">
              <div className="h-9 w-9 rounded-full bg-red-600/15 ring-1 ring-red-600/40 flex items-center justify-center shrink-0">
                <Circle className="h-4 w-4 text-red-500 fill-red-500" aria-hidden="true" />
              </div>
              <div className="flex flex-col leading-tight min-w-0">
                <span className="text-white font-bold tracking-tight truncate">ReliefAI</span>
                <span className="text-xs text-red-500 truncate">Disaster Response</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <div className="relative hidden sm:block">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-md bg-white/5 px-3 py-2 text-sm text-gray-200 hover:bg-white/10 transition-colors ring-1 ring-white/10"
                ...
                {selectorOpen ? (
                  <div
                    className="absolute right-0 mt-2 w-64 rounded-lg bg-gray-950 ring-1 ring-white/10 shadow-lg overflow-hidden"
                    role="listbox"
                  >
                    <div className="px-3 py-2 text-xs uppercase tracking-wide text-gray-400 border-b border-white/10">
                      Select disaster
                    </div>
                    <ul className="py-1">
                      {disasters.map((d) => {
              ...
              >
                {mobileOpen ? (
                  <X className="h-5 w-5 text-gray-200" aria-hidden="true" />
                ) : (
                  <Menu className="h-5 w-5 text-gray-200" aria-hidden="true" />
                )}
              </button>
            </div>
          </div>
          ...
          <div className="md:hidden border-t border-white/10">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 py-3 flex flex-col gap-2">
                ...
                {NAV_LINKS.map((l) => {
                  ...
                  return (
                    <Link
                      ...
                      className={[
                        "px-3 py-2 rounded-md text-sm font-medium transition-colors",
                        active ? "text-white bg-white/10" : "text-gray-200 hover:bg-white/5",
                      ].join(" ")}
                      aria-current={active ? "page" : undefined}
                    >
                      {l.label}
              ...
              <div className="pt-2 border-t border-white/10">
                <div className="text-xs uppercase tracking-wide text-gray-400 mb-2">Disaster</div>
                <div className="grid gap-1">
                  {disasters.map((d) => {
                    const active = d === disasterType;
                    return (
      ...
      <div className="w-full bg-red-600 text-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="h-10 flex items-center text-sm font-semibold tracking-tight">
